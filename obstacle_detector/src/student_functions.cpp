...
// {
//     cv::Mat output(map_size, CV_8UC1);
//     for (auto r = 0; r < output.rows; r++){
//         for (auto c = 0; c < output.cols; c++){
//             const cv::Vec3d dest_vec(c, r, 1);
//             const cv::Vec3d src_vec = cv::Mat1d(homography.inv() * dest_vec);

//             const cv::Point2i dest_point(c, r);
//             const cv::Point2i src_point(src_vec[0] / src_vec[2], src_vec[1] / src_vec[2]); //to renormalize homogenous coordinates
